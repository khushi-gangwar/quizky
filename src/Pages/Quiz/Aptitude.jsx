...
const Aptitude = () => {
  ...
  const changeQuestion = () => {
    if (currentQuestion < AptitudeData.length - 1) {
  ...
  const updateScore = () => {
    if (ClickedOption === AptitudeData[currentQuestion].answer) {  //if clicked option is same as ans inc score
      setscore(score + 1);
    }
  };
  const resetAll=()=>{
  ...
  return (
    ...
    <div className="main">
        ...
        {showResult ? (
          <QuizResult
            score={score}
            totalScore={AptitudeData.length}
            tryAgain={resetAll} 
          />
        ) : (
          <>
    ...
    <div className="question">
          ...
          <span id="question-txt">{AptitudeData[currentQuestion].question}</span>
        </div>
        <div className="option-container">
          {AptitudeData[currentQuestion].options.map((options, i) => {
            return (
              <button
